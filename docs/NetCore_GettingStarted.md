...
1. Navigate to the root of the Roslyn-analyzers repo and run these commands:
    - `cd roslyn-analyzers`
    - Set RUNTIMEPACKAGEVERSION into which ever version runtime/roslyn-analyzers repo is using: `set RUNTIMEPACKAGEVERSION=5.0.0`
...
10. In case you want to [debug](#Debugging-analyzer-with-runtiem-repo-projects) some failures.
...
1. Copy over debug build of analyzer assemblies on top of NetAnalyzers nuget package in your packages folder. (You might have alreayd did it in the [Testing against the Runtime repo step](#Testing-against-the-Runtime-and-Roslyn-analyzers-repo), if not do step 1 and 2 of that section)
2. Start VS and open runtime repo project you want to debug
3. Note the process ID for "ServiceHub.RoslynCodeAnalysisService.exe" corresponding to that VS instance
4. Set breakpoints in analyzer solution where you want to debug
5. Open another VS instance for RoslynAnalyzers.sln and attach to above process ID
6. Start typing in runtime repo project, the breakpoints should hit
  - If breakpoints are not hitting then the RoslynAnalyzers.sln build are not same as the build you copied to the step 1 repeat the step again or check if you copied into correct path
